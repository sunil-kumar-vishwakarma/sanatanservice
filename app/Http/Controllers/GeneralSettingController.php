...
class GeneralSettingController extends Controller
{
    public function general()
    {
        ...
        return view('admin.g-setting.general');
    }
    public function payments()
    {
        // You can fetch astrologer data from the database here and pass it to the view
        return view('admin.g-setting.payments');
    }
    public function social_link()
    {
        // You can fetch astrologer data from the database here and pass it to the view
        return view('admin.g-setting.social_link');
    }
    public function third_party_package(Request $req)
    {
        try {
			
            // if (Auth::guard('web')->check()) {
                // $flagGroup = DB::table('flaggroup')->whereNull('parentFlagGroupId')->get();
                $flagGroup = DB::table('flaggroup')->where('id','6')->get();
                for ($i = 0; $i < count($flagGroup); $i++) {
                    ...
                    if ($subGroup && count($subGroup) > 0) {
                        ...
                        $systemFlag = DB::table('systemflag')->where('flagGroupId', '6')->get();
                ...
                $systemFlagData = DB::table('systemflag')->get();

                // print_r($astroApiCallType);die;
                return view('admin.g-setting.third_party_package', compact('flagGroup', 'language', 'astroApiCallType','systemFlagData'));
            // } else {
        ...
        // return view('admin.g-setting.third_party_package');
    }
    public function master_image()
    {
        // You can fetch astrologer data from the database here and pass it to the view
        return view('admin.g-setting.master_image');
    }
    public function website_config()
    {
        // You can fetch astrologer data from the database here and pass it to the view
        return view('admin.g-setting.website_config');
    }
